...
Answer:
They share a positive relationship with an average scale of 1g to 10g in favor of male mass.
## Parrots
...
ggplot(birds, aes(male_mass, egg_mass)) + 
  ...
  annotate("text", 100, 50, label = "Red points are parrots") 
...
ggplot(birds, aes(male_mass, egg_mass)) + 
  ...
  scale_y_log10() +
  geom_smooth(method = "lm", color = "black", se = FALSE, linetype = "dashed", size = 0.5) + 
  annotate("text", 87, 218.7, label = "Red-tailed Tropical Bird")+
  annotate("point", 87, 218.7, color = "blue")
```
...
```{r}
thornbill <- filter(birds, type == "Thornbill")
thornbill
...
```{r}
ggplot(birds,aes(clutch_size, egg_mass))+
  geom_point()+
  scale_x_log10() +
  scale_y_log10() +
  geom_point(color = "red", data = thornbill) + 
  annotate("text", 5, 50, label = "Red points are Thornbills") +
  ggtitle("Thornbill egg masses to cluth size ratios")
```
