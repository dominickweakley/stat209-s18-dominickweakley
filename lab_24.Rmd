...
**Answer**:This is the average number of arrests per area number.
...
**Answer**The average number of thefts per area is .27
...
**Answer**:This means that there are 38816 total thefts in the data set. 
...
**Answer**:It means there are 36.55 crimes per 1000 households
...
**Answer**:This is smaller than the mean over the entire data set. We can not compare these numbers to find a relationship.
...
**Answer**:This is smaller than the mean over the entire dataset.  
...
**Answer**:This is larger than the thefts over the entire dataset.  We can compare these measurements as the thefts over the entire data set in about 8 thefts less.
...
```{r}
temp2 <- filter(ca, area_number == 23) $num_households
```
...
```{r}
crimes_ca <- left_join(ca, crimes_ca)
```
...
**Answer**: In general the higher the median income the lower the theft rate.  There is one oulier that has the highest theft rate and a very high median income.
...
**Answer**:The neighborhood has such a high crime rate due to it beinga high commercial zone that sees a lot of foot traffic.  It also is the business district of chicago meaning that there is not a lot of homes in the area.
