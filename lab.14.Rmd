...
```{r}
types_birds <- group_summarize(birds)
types_birds
```
...
```{r}
ggplot(new_birds, aes(egg_mass_median, male_mass_median)) +
...
```{r}
new_birds <- group_summarize(birds, type)
print(new_birds, n >= 5)
```
...
```{r}
ggplot(new_birds, aes(type, egg_mass_median)) +
  geom_point(aes(color = cut(male_mass_median, 5))) +
  annotate("rect", xmin = 0, xmax = 170,
           ymin = 0, ymax = 100,
           fill = "purple", alpha = 0.1) +
  annotate("text", x = 85, y = 50,
           label = "Smaller sets of Birds") 
